...
function FusoHorarioPadrao(UF: String): String;
// Retorna o fuso hor�rio padr�o para a UF
// Abaixo as UF est�o ordenadas por regi�o (Sul, sudeste, Centro-Oeste, Norte, Nordeste)
begin
  Result := '';
  UF := AnsiUpperCase(UF);
  //Sul
  ...
  if UF = 'MS' then // Mato Grosso do Sul
    Result := '-04:00';
  if UF = 'DF' then // Bras�lia (DF)    Hora Legal
    Result := '-03:00';
  ...
  if UF = 'CE' then // Cear�
    Result := '-03:00';
  if UF = 'MA' then // Maranh�o
    Result := '-03:00';
  if UF = 'PB' then // Para�ba
    Result := '-03:00';
...
function DefineFusoHorario(ArquivoIni: String; SecaoIni: String; ChaveIni: String; sUF: String; sFuso: String; bHorarioVerao: Boolean): String;
// Define o fuso hor�rio no arquivo .ini de configura��es
// Exemplos de uso:
// DefineFusoHorario('nfe.ini','nfe','fuso', 'SC', '', True); // Definir� autom�ticamente o fuso hor�rio com -03:00, que o fuso de Santa Catarina e em seguida Habilita Hor�rio de Ver�o. Usar esta maneira ap�s usu�rio configurar o emitente, quando ainda n�o h� fuso hor�rio definido.
// DefineFusoHorario('nfe.ini','nfe','fuso', '', '-02:00'); // Desconsidera a UF e define o fuso hor�rio que quiser. Usar esta maneira na rotina de configura��o de fuso hor�rio.
//
var
  Ini: TIniFile;
begin
...
// Habilita ou desabilita o fuso hor�rio para o hor�rio de ver�o
// Trata apenas a condi��o do Brasil, onde todos os fusos s�o Negativos
// Exemplo de uso:
// HabilitaHorarioVerao('nfe.ini','nfe','fuso', 'SC', True); Ativa hor�rio de ver�o para o fuso
// HabilitaHorarioVerao('nfe.ini','nfe','fuso', 'SC', False); Desativa hor�rio de ver�o para o fuso
var
  Ini: TIniFile;
  sFuso: String;
begin
  ...
  begin
    ...
    if bHabilita then // Entrando no hor�rio de ver�o
...
procedure FecharAplicacao(sNomeExecutavel: String);
begin
  //Mais eficiente para encerrar as aplica��es
...
begin
  try
    {acumula a soma da multiplica��o dos digitos}
    Pp1:=alltrim(pP1);
    Controle:=2;
    Acumulado:=0;
...
begin
   ...
   begin
     if Result >= 0 then
       if (StrToInt(Copy(sTemp,I,1))) > 1 then Result:=-1 else // se o n for > 1 n�o � bin�rio
...
{--- fim da fun��o PrimeiraMai�scula ---}
...
begin
  ...
  begin
    ...
    if length(pP1) = 14 then
       begin
          DIGITOS := Copy(pP1, 13, 2); {d�gitos informados}
          MULT := '543298765432';
          CONTROLE := '';
          {loop de verifica��o}
          {J := 1;}
          for J := 1 to 2 do
           ...
           {compara os d�gitos calculados(CONTROLE)}
           {com os d�gitos informados (DIGITOS)}
    ...
    {se for CPF}
       ...
       begin
          DIGITOS := Copy(pP1, 10, 2); {d�gitos informados}
          ...
          for J := 1 to 2 do
           begin
              SOMA := 0;
              K:=0; {coloquei para n�o dar erro}
              for I := 1 to 9 do
                begin
                   if I = 1 then K:=1
                   else
...
begin
  ...
  begin
    if Pos(AnsiUpperCase(Copy(sTexto, I, 1)), '1234567890ABCDEFGHIJKLMNOPQRSTUVXYZW!@#$%*()_+-=;:/|\?,.��') > 0 then
...
{Fun��o PACK}
{par�metros: pP1= nome do DataBase ou diret�rio, pP2=nome do arquivo,}
Function Pack(pP1:TTable):boolean;
var
  ArquivoDbf:file;
  NomeDbf:String;
...
{Fun�ao STRTRAN busca e troca uma parte de uma string em outra}
...
{fun��o EXTENSO
 pP1 valor num�rico, pP2 moeda (real, dolar), retorna uma string}
...
begin
   ...
   R:=StrTran(R,'e e',        'e');
   R:=StrTran(R,'�es e',      '�es');
   R:=StrTran(R,'um milh�es',  'um milh�o');
   R:=StrTran(R,'um bilh�es',  'um bilh�o');
   R:=StrTran(R,'um trilh�es', 'um trilh�o');
...
begin
  ...
  Result:=StrTran(Result,'dez e tr�s',  'treze');
  ...
  Result:=StrTran(Result,'e e',        'e');
  Result:=StrTran(Result,'�es e',      '�es');
  Result:=StrTran(Result,'um milh�es',  'um milh�o');
  Result:=StrTran(Result,'um bilh�es',  'um bilh�o');
  Result:=StrTran(Result,'um trilh�es', 'um trilh�o');
...
begin
   ...
   begin
      Result:=Copy('Janeiro  FevereiroMar�o    Abril    Maio     Junho    '+
...
begin
  ...
  begin
     if (I mod 2) = 0 then // posicao par s� acumula
  ...
  if (CalcCartao < 150) and ((CalcCartao mod 10) = 0) then //se o resto da divis�o por 10 for zero
    Result:=True
  else
    Result:=False;
end;
...
begin
  ...
  begin
    if (Copy(sNumero,3,2) >= '96') and (Copy(sNumero,3,2) <= '99') then Result := 'Amaz�nia celular - Amaz�nia';
    if (Copy(sNumero,3,2) >= '91') and (Copy(sNumero,3,2) <= '94') then Result := 'Vivo - Amaz�nia';
    if (Copy(sNumero,3,2) >= '86') and (Copy(sNumero,3,2) <= '89') then Result := 'Oi - Amaz�nia';
    if (Copy(sNumero,3,2) >= '81') and (Copy(sNumero,3,2) <= '83') then Result := 'Tim - Amaz�nia';
  end;
  //
  // Minas Gerais
  //
...
// Ver
  ...
  begin
    ...
    begin
      if (Copy(sNumero,3,3) >= '996') and (Copy(sNumero,3,3) <= '999') then Result := 'Vivo - S�o Paulo';
      if (Copy(sNumero,3,3) >= '971') and (Copy(sNumero,3,3) <= '974') then Result := 'Vivo - S�o Paulo';
      if (Copy(sNumero,3,3) >= '991') and (Copy(sNumero,3,3) <= '994') then Result := 'Claro - S�o Paulo';
      if (Copy(sNumero,3,3) >= '981') and (Copy(sNumero,3,3) <= '986') then Result := 'Tim - S�o Paulo';
      if (Copy(sNumero,3,3) = '995')                                  then Result := 'Vivo - S�o Paulo';
      if (Copy(sNumero,3,3) = '976') and (Copy(sNumero,3,3) = '989')   then Result := 'Claro - S�o Paulo';
    end else
    begin
      if (Copy(sNumero,3,2) >= '96') and (Copy(sNumero,3,2) <= '99') then Result := 'Vivo - S�o Paulo';
      if (Copy(sNumero,3,2) >= '71') and (Copy(sNumero,3,2) <= '74') then Result := 'Vivo - S�o Paulo';
      if (Copy(sNumero,3,2) >= '91') and (Copy(sNumero,3,2) <= '94') then Result := 'Claro - S�o Paulo';
      if (Copy(sNumero,3,2) >= '81') and (Copy(sNumero,3,2) <= '86') then Result := 'Tim - S�o Paulo';
      if (Copy(sNumero,3,2) = '95')                                  then Result := 'Vivo - S�o Paulo';
      if (Copy(sNumero,3,2) = '76') and (Copy(sNumero,3,2) = '89')   then Result := 'Claro - S�o Paulo';
    end;
    //
  end;
  //
  ...
  begin
    if (Copy(sNumero,3,4) = '9911')                                    then Result := 'Tim - Pelotas e regi�o';
    if (Copy(sNumero,3,4) = '9913')                                    then Result := 'Tim - Pelotas e regi�o';
    if (Copy(sNumero,3,4) = '9939')                                    then Result := 'Tim - Pelotas e regi�o';
    if (Copy(sNumero,3,4) >= '9981') and (Copy(sNumero,3,4) <= '9989') then Result := 'Tim - Pelotas e regi�o';
  end;
  //
end;
...
begin
   ...
   begin
      ...
      if UF='TO' then // deve-se extrair dois d�gitos
         // se n�o for os n�meros abaixo, deixa com 11 d�gitos e bTamanho ficar� falso
          ...
          if UF='DF' then
           ...
           if UF='MG' then
            ...
            if UF='SP' then
             ...
             if UF='PR' then
              begin
                if (length(sSoNumero)=10) then //inscri��o nova
                 begin
                 ...
                 begin
                   if (length(sSoNumero)=8) then // inscri��o Antiga
...
function DiaDaSemana(pP1:TDateTime):String;
begin
  Result := Copy('DomingoSegundaTer�a  Quarta Quinta Sexta  S�bado ',((DayOfWeek(pP1)-1)*7)+1,7);
end;
