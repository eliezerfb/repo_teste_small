...
uses Mais, Unit34, Unit30, Unit16, Mais3, Unit2, uFuncoesRetaguarda,
  ...
  uArquivosDAT, uDialogs, uSmallResourceString, uSmallConsts;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Form7.sModulo = 'Relatório de produtos monofásicos (NF-e)' then
        begin
          RelatorioProdutosMonofasicos(F,dInicio,dFinal);
        end;

        if Form7.sModulo = 'Relatório de PIS/COFINS (NF-e)' then
        begin
          RelatorioPisCofinsNfe(F,dInicio,dFinal);
        end;

        if Form7.sModulo = 'Relatório de PIS/COFINS (Cupom Fiscal)' then
        begin
        ...
        if Form7.sModulo = 'Relatório de IPI' then
        begin
          RelatorioIPI(F,dInicio,dFinal);
        end;

        if Form7.sModulo = 'Relatório de serviços' then
        begin
          RelatorioServicos(F,dInicio,dFinal);
        end;

        if Form7.sModulo = 'Relatório de serviços por técnico' then
        begin
          RelatorioServicosTecnico(F,dInicio,dFinal);
        end;

        if Form7.sModulo = 'Relatório de vendas por vendedor' then
        begin
          RelatorioVendasVendedor(F,dInicio,dFinal);
        end;

        if Form7.sModulo =  'Previsão de compras' then
        begin
        ...
        if Form7.sModulo =  'Peças em OS abertas' then
        begin
        ...
        if (Form7.sModulo =  'Relatório de produtos monofásicos (Cupom Fiscal)') then
        begin
          RelatorioProdutosMonofasicosCupom(F,dInicio,dFinal);
        end;

        if Form7.sModulo =  'Relatório de vendas (Cupom Fiscal)' then
        begin
          RelatorioVendasCupom(F,dInicio,dFinal);
        end;

        if Form7.sModulo =  'Relatório de orçamentos pendentes' then
        begin
          RelatorioOrcamentosPendentes(F,dInicio,dFinal);
        end;

        if (Form7.sModulo =  'Relatório de compras') or (Form7.sModulo =  'Relatório de vendas') then
        begin
...
begin
  ...
  while not Form7.ibDataSet99.Eof do
  begin
    if (AllTrim(Form7.ibDAtaSet99.FieldByname('DESCRICAO').AsString) <> 'Desconto') and (AllTrim(Form7.ibDAtaSet99.FieldByname('DESCRICAO').AsString) <> 'Acrécimo') then
    begin
...
begin
  ...
  while not Form7.ibDataSet100.Eof do
  begin
    if (AllTrim(Form7.ibDAtaSet100.FieldByname('DESCRICAO').AsString) <> 'Desconto') and (AllTrim(Form7.ibDAtaSet100.FieldByname('DESCRICAO').AsString) <> 'Acréscimo') then
    begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Writeln(F,'   <br><font face="Microsoft Sans Serif" size=1>ORÇAMENTO '+Form7.ibDataSet37PEDIDO.AsString+' '+DateToStr(Form7.ibDataSet37DATA.AsDateTime)+' '+Form7.ibDataSet37VENDEDOR.AsString);   // Linha de talhe
        WriteLn(F,'  </td>');
        WriteLn(F,' </tr>');
        WriteLn(F,' <tr>');
        WriteLn(F,'  <td>');
      ...
      begin
        ...
        Writeln(F,'ORÇAMENTO '+Form7.ibDataSet37PEDIDO.AsString+' '+DateToStr(Form7.ibDataSet37DATA.AsDateTime)+' '+Form7.ibDataSet37VENDEDOR.AsString);   // Linha de talhe
        WriteLn(F,'');
