...
uses
  ...
  // Sandro Silva procedure ImportaOrcamento(NumeroOrcamento:string; sTipo : String);
  function ImportaOrcamento(NumeroOrcamento: String; sTipo : String): Boolean;
...
//Sandro Silva 2024-10-10 procedure ImportaOrcamento(NumeroOrcamento:string; sTipo : String);
function ImportaOrcamento(NumeroOrcamento: String; sTipo : String): Boolean;
var
  iDupl, I, iB : Integer;
  sReg, sEstado : String;
  ...
  slCompostoPai: TStringList; // Sandro Silva 2024-10-10
  sMensagemComposicao: String;
  IBQORCAMENT: TIBQuery;
begin
  {Sandro Silva 2024-10-10 inicio}
  Result := True;
  try
    // Valida se o produto é composto e tem composição circular (quando o produto principal é insumo de um dos insumos)

    IBQORCAMENT := CriaIBQuery(Form7.ibDataSet37.Transaction);
    IBQORCAMENT.Close;
    IBQORCAMENT.SQL.Text := ' Select * from ORCAMENT'+
                                        ' Where PEDIDO='+QuotedStr(NumeroOrcamento)+
                                        '   and coalesce(NUMERONF,'''') = '''' '+   //Mauricio Parizotto 2023-10-16
                                        ' Order by REGISTRO';
    IBQORCAMENT.Open;
    IBQORCAMENT.First;
    //slCompostoPai := TStringList.Create; // Sandro Silva 2024-10-10
    while not IBQORCAMENT.Eof do
    begin
      //slCompostoPai.Clear;
      if IBQORCAMENT.FieldByName('CODIGO').AsString <> '' then
      begin
        //slCompostoPai.Add(IBQORCAMENTCODIGO.AsString);
        //if ComposicaoCircular(IBQORCAMENTCODIGO.AsString, IBQORCAMENT.Transaction, slCompostoPai) then
        if ProdutoTemComposicaoCircular(IBQORCAMENT.FieldByName('CODIGO').AsString, IBQORCAMENT.Transaction) then
        begin
          sMensagemComposicao := sMensagemComposicao +
            IBQORCAMENT.FieldByName('CODIGO').AsString + ' - ' + IBQORCAMENT.FieldByName('DESCRICAO').AsString + #13 + #13;
          Result := False;
        end;
      end;
      IBQORCAMENT.Next;
    end;
    FreeAndNil(IBQORCAMENT);
    //FreeAndNil(slCompostoPai);

    if sMensagemComposicao <> '' then
    begin
      MensagemSistema('Orçamento ' + NumeroOrcamento + ' não será importado ' + #13 +
        'Produto(s) com composição circular' + #13 + #13 +
        'Corrija a composição' + #13 + #13 +
        sMensagemComposicao, msgAtencao);
      Exit;
    end;
