...
begin
  try
    ...
    {Mauricio Parizotto 2024-04-25 Inicio}
    Form1.sArquivoRemessa := Copy(StrTran(DateToStr(Date),'/','_')+DiaDaSemana(Date)+replicate('_',10),1,14)+StrTran(TimeToStr(Time),':','_')+'.txt';
    ...
    if Copy(AllTrim(Form26.MaskEdit42.Text),1,3) = '077' then // Inter
    begin
      Form1.sArquivoRemessa := 'CI400_001_'+StrZero(iRemessa,7,0)+'.REM';
    end;

    AssignFile(F,Form1.sAtual+'\remessa\'+Form1.sArquivoRemessa);
    Rewrite(F);   // Abre para gravação
    {Mauricio Parizotto 2024-04-25 Fim}


    try
      {$Region'//// SICOOB HEADER ////'}
      ...
      {$Region'//// BRADESCO //// '}
      if Copy(AllTrim(Form26.MaskEdit42.Text),1,3) = '237' then
      begin
        WriteLn(F,
      ...
      begin
        WriteLn(F,
          '0'                                                                                       + // 01 001 a 001 (001) Identificação do Registro Header: “0” (zero)
          '1'                                                                                       + // 02 002 a 002 (001) Tipo de Operação: “1” (um)
          'REMESSA'                                                                                 + // 03 003 a 009 (007) Identificação por Extenso do Tipo de Operação 01
          '01'                                                                                      + // 04 010 a 011 (002) Identificação do Tipo de Serviço: “01”
          Copy('COBRANCA'+Replicate(' ',15),1,15)                                                   + // 05 012 a 026 (015) Identificação por Extenso do Tipo de Serviço: “COBRANCA”
          Replicate(' ',20)                                                                         + // 06 027 a 046 (020) Complemento do Registro: “Brancos”
          Copy(UpperCase(Form7.IbDataSet13NOME.AsString)+Replicate(' ',30),1,030)                   + // 07 047 a 076 (030) Nome da Empresa
          '077'                                                                                     + // 08 077 a 079 (003) "077"
          Copy('INTER'+Replicate(' ',15),1,15)                                                      + // 09 080 a 094 (015) Nome Banco
          Copy(Copy(DateToStr(Date),1,2)+Copy(DateToStr(Date),4,2)+Copy(DateToStr(Date),9,2),1,006) + // 10 095 a 100 (006) Data da Gravação: Informe no formato “DDMMAA” 21
          Replicate(' ',10)                                                                         + // 11 101 a 110 (010) Complemento do Registro: “Brancos”
          Copy(StrZero(iRemessa,7,0),1,007)                                                         + // 12 111 a 117 (007) Seqüencial da Remessa
          Replicate(' ',277)                                                                        + // 13 118 a 394 (277) Complemento do Registro: “Brancos”
          Copy('000001',1,006)                                                                      + // 14 395 a 400 (006) Seqüencial do Registro:”000001”
          ''
          );
      end;
      {$Endregion}
          ...
          try
            ...
            try
              ...
              try
                ...
                {$Region'//// CAIXA ////'}
                if Copy(AllTrim(Form26.MaskEdit42.Text),1,3) = '104' then
                begin
                ...
                if Copy(AllTrim(Form26.MaskEdit42.Text),1,3) = '077' then
                begin
                  sConta   := Right('000000000'+Copy(Form7.ibDataSet11CONTA.AsString,1,Pos('-',Form7.ibDataSet11CONTA.AsString)-1),9);
                  sDVConta := Copy(Copy(Form7.ibDataSet11CONTA.AsString,Pos('-',Form7.ibDataSet11CONTA.AsString)+1,1)+' ',1,1);
                  ...
                  WriteLn(F,
                    ...
                    Replicate('0',25)                                                                        + // 038 a 062 - (25) Número de controle para uso da empresa
                    Replicate(' ',3)                                                                         + // 063 a 065 - (03) Branco
                    '0'                                                                                      + // 066 a 066 - (01) Multa 0- sem 1- fixo 2- percentual
                    Replicate('0',13)                                                                        + // 067 a 079 - (13) Valor Multa
                    Replicate('0',4)                                                                         + // 080 a 083 - (04) Percentual Multa
                    ...
                    Copy(Copy(DateToStr(Form7.ibDataSet7VENCIMENTO.AsDateTime),1,2)                          +
                    Copy(DateToStr(Form7.ibDataSet7VENCIMENTO.AsDateTime),4,2)                               +
                    Copy(DateToStr(Form7.ibDataSet7VENCIMENTO.AsDateTime),9,2),1,006)                        + // 121 a 126 - (06) DATA DE VENCIMENTO DO TÍTULO
                    Copy(StrZero((Form7.ibDataSet7VALOR_DUPL.AsFloat * 100),13,0),1,013)                     + // 127 a 139 - (13) VALOR NOMINAL DO TÍTULO
                    '60'                                                                                     + // 140 a 141 - (02) Data limite de pagamento
                    Replicate(' ',6)                                                                         + // 142 a 147 - (03) Branco
                    '01'                                                                                     + // 148 a 149 - (02) Espécie do Título
                    'N'                                                                                      + // 150 a 150 - (01) Identificação
                    Copy(Copy(DateToStr(Form7.ibDataSet7EMISSAO.AsDateTime),1,2)                             +
                    Copy(DateToStr(Form7.ibDataSet7EMISSAO.AsDateTime),4,2)                                  +
                    Copy(DateToStr(Form7.ibDataSet7EMISSAO.AsDateTime),9,2),1,006)                           + // 151 a 156 - (06) DATA DA EMISSÃO DO TÍTULO
                    Replicate(' ',3)                                                                         + // 157 a 159 - (03) Branco
                    '0'                                                                                      + // 160 a 160 - (02) "0" = Sem juros/mora "1" = Valor fixo de juros/mora por dia "2" = Taxa mensal de juros/mora (cobrado pro rata)
                    Replicate('0',13)                                                                        + // 161 a 173 - (13) Valor a ser Cobrado por Dia de Atraso
                    Replicate('0',4)                                                                         + // 174 a 177 - (04) Valor a ser Cobrado por Dia de Atraso
                    Replicate('0',6)                                                                         + // 178 a 183 - (06) Data da mora
                    '0'                                                                                      + // 184 a 184 - (02) "0" = Sem desconto "1" = Valor fixo até a data informada ”4" = Percentual do valor nominal até a data informada.
                    Replicate('0',13)                                                                        + // 185 a 197 - (13) Valor desconto
                    Replicate('0',4)                                                                         + // 198 a 201 - (04) Percentual de Desconto
                    Replicate('0',6)                                                                         + // 202 a 207 - (04) Data limite para Concessão do desconto
                    Replicate(' ',13)                                                                        + // 208 a 220 - (13) Branco
                    Copy(sCPFOuCNPJ,1,002)                                                                   + // 221 a 222 - (02) IDENTIFICAÇÃO DO TIPO DE INSCRIÇÃO/SACADO
                    Copy(Right(Replicate('0',14)+LimpaNumero(Form7.IBDataSet2CGC.AsString),14),1,14)         + // 223 a 236 - (14) Nº DE INSCRIÇÃO DO SACADO (CPF/CNPJ)
                    Copy(Form7.ibDataSet2NOME.AsString+Replicate(' ',40),1,40)                               + // 237 a 276 - (40) Nome do Pagador
                    Copy(Form7.ibDataSet2ENDERE.AsString+Replicate(' ',38),1,38)                             + // 277 a 314 - (38) Endereço Completo do Pagador
                    Copy(Form7.ibDataSet2ESTADO.AsString+Replicate(' ',2),1,2)                               + // 315 a 316 - (02) UF Do Pagador
                    Copy(LimpaNumero(Form7.ibDataSet2CEP.AsString)+Replicate('0',8),1,8)                     + // 317 a 324 - (08) CEP Do Pagador
                    Replicate(' ',70)                                                                        + // 325 a 394 - (70) Mensagem 1
                    Copy(StrZero(iReg,6,0),1,6)                                                              + // 395 a 400 - 9(06) Nº SEQÜENCIAL DO REGISTRO NO ARQUIVO
                    ''
                    );
                end;
                {$Endregion}
    ...
    {$Region'//// Inter TRAILER ////'}
    if (Copy(AllTrim(Form26.MaskEdit42.Text),1,3) = '077')then
    begin
      WriteLn(F,
        Copy('9',1,001)                                                                                                            + // 001 a 001 - (001)      Código do registro = 9
        Copy(StrZero(iReg-1,6,0),1,006)                                                                                            + // 002 a 007 - (006)      Quantidade de Boletos
        Replicate(' ',387)                                                                                                         + // 008 a 020 - (011)      Branco
        Copy(StrZero(iReg+1,6,0),1,006)                                                                                            + // 395 a 400 - (006)      Número sequencial do registro no arquivo
        ''
        );
    end;
    {$Endregion}

    {$Region'//// Trailer outros'}
    if not((Copy(AllTrim(Form26.MaskEdit42.Text),1,3) = '033') or (Copy(AllTrim(Form26.MaskEdit42.Text),1,3) = '353'))
      and not(Copy(AllTrim(Form26.MaskEdit42.Text),1,3) = '756')
      and not (Copy(AllTrim(Form26.MaskEdit42.Text),1,3) = '041')
      and not (Copy(AllTrim(Form26.MaskEdit42.Text),1,3) = '077')  then
    begin
      WriteLn(F,
