...
uses Unit7, Mais, Unit12, Unit34, Unit13, Unit33;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        Form7.ibDataSet27.SelectSQL.Add('select * from ALTERACA where DATA='+QuotedStr(DAteToStrInvertida(DATE))+' order by PEDIDO');
...
//      Form7.ibDataSet37.SelectSQL.Add('select * from ORCAMENT order by PEDIDO');
      Form7.ibDataSet37.Open;
      //
//      Form7.ibDataSet37.Last;
...
begin
  ...
  begin
    ...
    Form7.ibDataSet3.SelectSQL.Add('select * from OS where NUMERO='+QuotedStr(MaskEdit1.Text)+' ');
    ...
    begin
      ...
      Form7.ibQuery1.SQL.Add('select * from ITENS003 where NUMEROOS='+QuotedStr(Form7.ibDataSet3NUMERO.AsString)+' order by REGISTRO');
      Form7.ibQuery1.Open;
      //
      ...
      begin
        ...
        Form7.ibDataSet16.SelectSQL.Add('select * from ITENS001 where NUMEROOS='+QuotedStr(Form7.ibDataSet3NUMERO.AsString)+' ');
  ...
  begin
    ...
    Form7.ibDataSet37.Close;
    Form7.ibDataSet37.SelectSQL.Clear;
    Form7.ibDataSet37.SelectSQL.Add('select * from ORCAMENT where PEDIDO='+QuotedStr(MaskEdit1.Text)+' order by REGISTRO');
    Form7.ibDataSet37.Open;
    ...
    begin
      ...
      begin
        //
//        if Form7.ibDataSet37.FieldByname('NUMERONF').AsString <> Form7.ibDataSet15NUMERONF.AsString then  // Sonhei com a solução deste problema ontem a noite
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                //
                // ShowMessage(Form7.IbDataSet37DESCRICAO.AsString);
                //
                        ...
                        begin
                          ...
                          begin
                            ...
                            begin
                              ...
                              Form7.ibDataSet14.Close;
                              Form7.ibDataSet14.SelectSQL.Clear;
          //                    Form7.ibDataSet14.SelectSQL.Add('select * FROM ICM where (CFOP like ''5'') or (CFOP like ''6'') or (CFOP like ''7'')  or (Coalesce(CFOP,'+QuotedStr('')+')='+QuotedStr('')+') order by upper(NOME)');
                              Form7.ibDataSet14.SelectSQL.Add('select * from ICM where SubString(CFOP from 1 for 1) = ''5'' or  SubString(CFOP from 1 for 1) = ''6'' or SubString(CFOP from 1 for 1) = ''7''  or Coalesce(SubString(CFOP from 1 for 1),''X'') = ''X'' order by upper(NOME)');
                              Form7.ibDataSet14.Open;
  ...
  begin
    ...
    Form7.ibDataSet16.Edit;
    //
    if Form7.IbDataSet27.Eof then ShowMessage('Cupom fiscal não encontrado ou já importado.');
    //
    ...
    begin
      ...
      Form7.ibDataSet4.Open;
      //
      if (Form7.IbDataSet27DESCRICAO.AsString = Form7.ibDataSet4DESCRICAO.AsString)  and (AllTrim(Form7.IbDataSet27DESCRICAO.AsString) <> '') then
      begin
        //
        if (Form1.ConfNegat = 'Não') and (Form7.ibDataSet4QTD_ATUAL.AsFloat < Form7.IbDataSet27QUANTIDADE.AsFloat) and ((sTipo <> 'BALCAO') and (sTipo <> 'VENDA')) then
        begin
          ShowMessage('Não é possível efetuar a venda de '+Form7.IbDataSet27DESCRICAO.AsString+chr(10)
          +'só tem ' + Form7.ibDataSet4QTD_ATUAL.AsString + ' no estoque');
        end else
        begin
          //
          if (AllTrim(Form7.ibDataSet15CLIENTE.AsString) = '') and (AllTrim(Form7.IbDataSet27CLIFOR.AsString) <> '') then
          begin
            //
            Form7.ibDataSet15VENDEDOR.AsString  := Form7.IbDataSet27VENDEDOR.AsString;
            Form7.ibDataSet15CLIENTE.AsString   := Form7.IbDataSet27CLIFOR.AsString;
            //
            Form7.ibDataSet2.Close;
            Form7.ibDataSet2.Selectsql.Clear;
            Form7.ibDataSet2.Selectsql.Add('select * from CLIFOR where NOME='+QuotedStr(Form7.ibDataSet15CLIENTE.AsString)+' ');  //
          ...
          Form7.ibDataSet16.Edit;
          Form7.ibDataSet16UNITARIO.AsFloat   := Form7.IbDataSet27UNITARIO.AsFloat;
          Form7.ibDataSet16TOTAL.AsFloat      := Arredonda((Form7.IbDataSet27UNITARIO.AsFloat * Form7.IbDataSet27QUANTIDADE.AsFloat),StrToInt(Form1.ConfPreco));
          Form7.ibDataSet16IPI.AsFloat        := Form7.ibDataSet4IPI.AsFloat;
          //
          Grade(Form7.IbDataSet27QUANTIDADE.AsFloat);
          //
          ...
          //
          // Form7.ibDataSet4.Edit;
          // Form7.ibDataSet4QTD_ATUAL.AsFloat := Form7.ibDataSet4QTD_ATUAL.AsFloat + Form7.ibDataSet16QUANTIDADE.AsFloat; // Volta a quantidade pro estoque;
          // Form7.ibDataSet4.Post;
      ...
      begin
        //
        if Alltrim(Form7.IbDataSet27DESCRICAO.AsString) <> '<CANCELADO>' then
        begin
          if Alltrim(Form7.IbDataSet27DESCRICAO.AsString) = 'Desconto' then
          begin
            Form7.ibDataSet15.Edit;
            Form7.ibDataSet15DESCONTO.AsFloat := Form7.ibDataSet15DESCONTO.AsFloat + (Form7.IbDataSet27TOTAL.AsFloat * -1);
          end else
          begin
            //
            try
              Form7.ibDataSet35.Append;
              Form7.ibDataSet35DESCRICAO.AsString := Form7.IbDataSet27DESCRICAO.AsString;
              Form7.ibDataSet35QUANTIDADE.AsFloat := Form7.IbDataSet27QUANTIDADE.AsFloat;
              Form7.ibDataSet35.Edit;
              Form7.ibDataSet35UNITARIO.AsFloat   := Form7.IbDataSet27UNITARIO.AsFloat;
              Form7.ibDataSet35TOTAL.AsFloat      := Arredonda((Form7.IbDataSet27UNITARIO.AsFloat * Form7.IbDataSet27QUANTIDADE.AsFloat),StrToInt(Form1.ConfPreco));
              Form7.ibDataSet35.Post;
            except end;
            //
          end;
    ...
    Form7.IbDataSet7.SelectSQL.Add('select * from RECEBER where substring(DOCUMENTO from 1 for 9)='+QuotedStr(AllTrim(MaskEdit2.Text)+Copy(MaskEdit1.Text,1,6))+' ');
    ...
    Form7.IbDataSet7.Close;
    Form7.IbDataSet7.SelectSQL.Clear;
    Form7.IbDataSet7.SelectSQL.Add('select * from RECEBER where NUMERONF = ' + QuotedStr(Form7.ibDataSet15NUMERONF.AsString) + ' ');
    Form7.IbDataSet7.Open;
    Form7.IbDataSet7.First;
    //
    I := 0;
    ...
    //
    if I <> 0 then Form7.ibDataSet15DUPLICATAS.AsFloat := I;
    //
