...
type
   ...
   HH_Popup = record
      ...
      pszText : PChar; // used if idstring is 0
      ...
      pszFont : PChar; // facename, pointsize, charset, BOLD ITALIC UNDERLINE
   end; // HH_Popup

   HH_AKLINK = record
      cbStruct : integer; // struct size
      fReserved : bool; // set to false
      pszKeywords : PChar; // ; separated keywords
      pszUrl : PChar; // url to jump if no keywords found
      pszMsgText : PChar; // message if no kwd and url is NULL
      pszMsgTitle : PChar; // title ....
...
type
   HH_WinType = record
      ...
      pszType : PChar; // IO: Name of type of window
	...
	// The following members are only valid if HHWIN_PROP_TRI_PANE is set
      ...
      pszToc : PChar; // In: location of toc file
      pszIndex : PChar; // In: location of index file
      pszFile : PChar; // In: default location of html file
      pszHome : PChar; // IO: html file for home button
...
begin
     ...
     if (OCXHandle <> 0) then begin
        // HtmlHelpA := GetProcAddress(OCXHandle, ATOM_HTMLHELP_API_ANSI);
        HtmlHelpA := GetProcAddress(OCXHandle, 'HtmlHelpA');
     end; // The OCX was loaded into memory...
end; // initialization
