...
begin
  ...
  try
    ...
    if not InternetSetOption(pSession, INTERNET_OPTION_CONNECT_TIMEOUT, @FTimeOut, SizeOf(FTimeOut)) then
      raise ESmallHTTPReqResp.Create('Erro ao definir TimeOut de Conexão');
    ...
    try
      ...
      try
        ...
        begin
          ...
          raise ESmallHTTPReqResp.Create('Erro: Requisição não enviada.' +
...
begin
  ...
  try
    ...
    CertsSel := Certs.Select(WideString(SmallStr('Certificados Digitais disponível')),
