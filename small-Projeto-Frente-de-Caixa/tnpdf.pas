...
Type
  TPDFBrushStyle = (poSolid, poDashed, poBeveled, poInset, poUnderline);
...
type
  ...
  private
    ...
    ParentNum, ContentNum, ResourceNum, OutLinesNum, CatalogNum,
...
function TPrintPDF.EndDoc: String;
var
  i: Integer;
begin
  Result := '';
  ...
  {$ENDIF}
    ...
    on E: Exception do
    begin
      // Application.MessageBox(PChar('Não foi possível criar ' + FileName + #13 + #13 + E.Message), 'Atenção', MB_ICONWARNING + MB_OK);
      Result := 'Não foi possível criar ' + FileName + #13 + #13 + E.Message;
    end;
  end;
end;
...
begin
  ...
  StreamWriteStr(TempStream, '/CreationDate (D:' +
...
begin
  ...
  for i := 1 to NumberofImages do
    StreamWriteStr(TempStream, '/Im' + IntToStr(i) + ' ' +
      IntToStr(CurrentObjectNum + i) + ' 0 R');
  StreamWriteStr(TempStream, '>>');
  StreamWriteStr(TempStream, '/Font << ');

  for i := 1 to FontNumber do
    StreamWriteStr(TempStream, '/F' + IntToStr(i) + ' ' + FontNumberList.Strings
      [i - 1] + ' 0 R ');
...
begin
  ...
  for i := 1 to PageNumber do
  begin
    StreamWriteStr(TempStream, IntToStr(CurrentObjectNum + i + 1 +
      NumberofImages) + ' 0 R');
...
begin
  ...
  StreamWriteStr(TempStream, '/MediaBox [ 0 0 ' + IntToStr(PageWidth) + ' ' +
    IntToStr(PageHeight) + ']');
  StreamWriteStr(TempStream, '/Contents ' + PageNumberList.Strings
    [CurrentSetPageObject] + ' 0 R');
...
{$ENDIF}
  ...
  TempSize := TempStream.Size - StreamSize1 - StreamSize2 - Length('Stream') -
    Length('endstream') - 6;
  ...
  TempStream.Clear;
  StreamWriteStr(TempStream, IntToStr(CurrentObjectNum) + ' 0 obj');
  StreamWriteStr(TempStream, '<< /Length ' + IntToStr(CurrentObjectNum + 1)
    + ' 0 R');
...
begin
  ...
  for i := 0 to CurrentObjectNum - 2 do
    StreamWriteStr(TempStream, GetOffsetNumber(Trim(ObjectOffsetList.Strings[i])
      ) + ' 00000 n');
  ...
  StreamWriteStr(TempStream,
    Trim(ObjectOffsetList.Strings[CurrentObjectNum - 1]));
  PDF.Seek(0, soFromEnd);
  TempStream.SaveToStream(PDF);
end;
...
begin
  StreamWriteStr(sTempStream, IntToStr(x1) + ' ' +
    IntToStr((PageHeight - y1)) + ' m');
  StreamWriteStr(sTempStream, IntToStr(x2) + ' ' +
    IntToStr((PageHeight - y2)) + ' l');
...
begin
  ...
  StreamWriteStr(sTempStream, '/F' + IntToStr((Integer(Font.Name) + 1)) + ' ' +
    IntToStr(Font.Size) + ' Tf');
  StreamWriteStr(sTempStream, IntToStr(X) + ' ' + IntToStr((PageHeight - Y)
    ) + ' Td');
...
begin
  ...
  tempsy := ((PageHeight) / (WinProcs.GetDeviceCaps(GetDC(0), LOGPIXELSY)
    * 11.900));

  NumberofImages := NumberofImages + 1;
  StreamWriteStr(sTempStream, 'q');
  StreamWriteStr(sTempStream,
    IntToStr(trunc(ABitmap.Picture.Bitmap.Width * tempsx)) + ' 0 0 ' +
    IntToStr(trunc(ABitmap.Picture.Bitmap.Height * tempsy)) + ' ' + IntToStr(X)
    + ' ' + IntToStr(PageHeight - Y - trunc(ABitmap.Picture.Bitmap.Height *
    tempsy)) + ' cm');
...
begin
  ...
  begin
    tempsx := ((PageWidth) / (WinProcs.GetDeviceCaps(GetDC(0),
      LOGPIXELSX) * 10));
    tempsy := ((PageHeight) / (WinProcs.GetDeviceCaps(GetDC(0), LOGPIXELSY)
      * 11.900));
  end
  ...
  begin
    tempsx := ((PageWidth) / (WinProcs.GetDeviceCaps(GetDC(0),
      LOGPIXELSX) * 13));
    tempsy := ((PageHeight) / (WinProcs.GetDeviceCaps(GetDC(0),
      LOGPIXELSY) * 8));
  end;

  NumberofImages := NumberofImages + 1;
  StreamWriteStr(sTempStream, 'q');
  StreamWriteStr(sTempStream, IntToStr(trunc(ABitmap.Width * tempsx)) + ' 0 0 '
    + IntToStr(trunc(ABitmap.Height * tempsy)) + ' ' + IntToStr(X) + ' ' +
    IntToStr(PageHeight - Y - trunc(ABitmap.Height * tempsy)) + ' cm');
  StreamWriteStr(sTempStream, '/Im' + IntToStr(NumberofImages) + ' Do');
  StreamWriteStr(sTempStream, 'Q');
  SetBitmap(ABitmap);
end;
...
{$ELSE}
  ...
  TRY
    ...
    TRY
      ...
      BEGIN
        GetDIBits(DeviceContext, tmpBitmap.Handle, j, 1, ScanLine, BitmapInfo,
          DIB_RGB_COLORS);
...
{$ENDIF}
  ...
  begin
    with PalEntries[aa] do
      s := s + IntToHex(peRed, 2) + IntToHex(peGreen, 2) +
        IntToHex(peBlue, 2) + ' ';
  end;
  ...
  StreamWriteStr(pTempStream, '/ColorSpace [/Indexed /DeviceRGB 255 <' +
    s + '>]');
