...
type
  ...
  TSStringGrid = class(TStringGrid)
  private
...
constructor TSStringGrid.Create(Owner: TComponent);
begin
...
destructor TSStringGrid.Destroy;
begin
  FHorFonte.Free;
  FTitFonte.Free;
  FTecnicos.Free;
...
procedure TSStringGrid.GeraGrade;
var
   Ht,Vt,Tec,Sentenca: String;
   Day,Month,Year: Word;
   Dia,Mes,Ano: String;
   PTec,I,C: Integer;
...
begin
  ...
  QData.DatabaseName := Table.DatabaseName;
  Sentenca := 'Select NUMERO,CLIENTE,SITUACAO,DATA_PRO,HORA_PRO,TEMPO,TECNICO from ' + FTable.TableName +
  ' WHERE EXTRACT(DAY FROM DATA_PRO) = '+Dia+' AND EXTRACT(MONTH FROM DATA_PRO) = '+Mes+' AND EXTRACT(YEAR FROM DATA_PRO) = '+Ano;
  QData.SQL.Text := Sentenca;
  QData.Active := True;
  QData.First;
  C := 0;
  SetLength(Ordens, QData.RecordCount); // Seta o array com o tamanho igual a quantidade de registros
  while not QData.Eof do
  begin
    Tec := Trim(QData.FieldByName('TECNICO').AsString);
    PTec := Tecnicos.IndexOf(Tec);
    h := PTec +1;
    HoraI := StrToDateTime(QData.FieldByName('HORA_PRO').AsString);
    HoraF := StrToDateTime(QData.FieldByName('HORA_PRO').AsString) + StrToDateTime(QData.FieldByName('TEMPO').AsString);
    ...
    Ordens[C].Numero := QData.FieldByName('NUMERO').AsString;
    Ordens[C].Cliente := QData.FieldByName('CLIENTE').AsString;
    Ordens[C].Situacao := QData.FieldByName('SITUACAO').AsString;
    Ordens[C].Data_pro := QData.FieldByName('DATA_PRO').AsDateTime;
    Ordens[C].Hora_pro := QData.FieldByName('HORA_PRO').AsString;
    Ordens[C].Tempo := QData.FieldByName('TEMPO').AsString;
    Ordens[C].Tecnico := QData.FieldByName('TECNICO').AsString;
...
procedure TSStringGrid.SetTecnicos(ATecnicos: TStringList);
begin
  FTecnicos.Assign(ATecnicos);
end;

procedure TSStringGrid.SetHoras(AHoras: TStringList);
begin
  FHoras.Assign(AHoras);
end;

procedure TSStringGrid.SetSituacoes(ASituacoes: TStringList);
begin
  FSituacoes.Assign(ASituacoes);
end;

procedure TSStringGrid.SetSituacoesCor(ASituacoesCor: TStringList);
begin
  FSituacoesCor.Assign(ASituacoesCor);
end;


procedure TSStringGrid.AjustaHorario;
var
   i : Integer;
   HC: String;
begin
...
procedure TSStringGrid.CargaTecnicos;
begin
...
procedure TSStringGrid.SetCantoColor(Value: TColor);
begin
...
procedure TSStringGrid.SetHorariosColor(Value: TColor);
begin
...
procedure TSStringGrid.SetSelectColor(Value: TColor);
begin
...
procedure TSStringGrid.SetHorFonte(const Value: TFont);
begin
  FHorFonte.Assign(Value);
end;

procedure TSStringGrid.SetTitFonte(const Value: TFont);
begin
  FTitFonte.Assign(Value);
end;


procedure TSStringGrid.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  CR, AC, AR : Integer;
  C: TColor;
  HoraA,HoraI,HoraF: TTime;
...
procedure TSStringGrid.DrawCell(ACol, ARow: Integer; mRect: TRect;  AState: TGridDrawState);
var
  PC,CR: Integer;
  f,p: Double;
...
procedure TSStringGrid.SetTable(Value: TTable);
begin
   FTable := Value;
end;

function TSStringGrid.GetTable: TTable;
begin
  Result := FTable;
end;
...
procedure Register;
begin
  RegisterComponents('Small', [TSStringGrid]);
end;
