unit SmallStringGrid;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  Grids, dbtables,  IBDatabase, IBCustomDataSet, IBTable, IBQuery, Unit7, SmallFunc;
...
type
  ...
  TSmallStringGrid = class(TStringGrid)
  private
...
constructor TSmallStringGrid.Create(Owner: TComponent);
begin
...
destructor TSmallStringGrid.Destroy;
begin
  FHorFonte.Free;
  FTitFonte.Free;
  FTecnicos.Free;
...
procedure TSmallStringGrid.GeraGrade;
var
  Ht,Vt,Tec: String;
  Day,Month,Year: Word;
  Dia,Mes,Ano: String;
...
begin
  ...
  Form7.ibDataSet100.SelectSql.Add('select NUMERO,CLIENTE,SITUACAO,DATA_PRO,HORA_PRO,TEMPO,TECNICO from OS WHERE EXTRACT(DAY FROM DATA_PRO) = '+Dia+' AND EXTRACT(MONTH FROM DATA_PRO) = '+Mes+' AND EXTRACT(YEAR FROM DATA_PRO) = '+Ano);
  ...
  //
  SetLength(Ordens,C); // Seta o array com o tamanho igual a quantidade de registros
  //
  C := 0;
  Form7.ibDataSet100.First;
  while not Form7.ibDataSet100.Eof do
  begin
    ...
    try
      Tec := Trim(Form7.ibDataSet100.FieldByName('TECNICO').AsString);
      PTec := Tecnicos.IndexOf(Tec);
      h := PTec +1;
      //
      HoraI := StrToDateTime(Form7.ibDataSet100.FieldByName('HORA_PRO').AsString);
      HoraF := StrToDateTime(Form7.ibDataSet100.FieldByName('HORA_PRO').AsString) + StrToDateTime(Form7.ibDataSet100.FieldByName('TEMPO').AsString);
      //
      ...
      Ordens[C].Numero   := Form7.ibDataSet100.FieldByName('NUMERO').AsString;
      Ordens[C].Cliente  := Form7.ibDataSet100.FieldByName('CLIENTE').AsString;
      Ordens[C].Situacao := Form7.ibDataSet100.FieldByName('SITUACAO').AsString;
      Ordens[C].Data_pro := Form7.ibDataSet100.FieldByName('DATA_PRO').AsDateTime;
      Ordens[C].Hora_pro := Form7.ibDataSet100.FieldByName('HORA_PRO').AsString;
      Ordens[C].Tempo    := Form7.ibDataSet100.FieldByName('TEMPO').AsString;
      Ordens[C].Tecnico  := Form7.ibDataSet100.FieldByName('TECNICO').AsString;
...
procedure TSmallStringGrid.SetTecnicos(ATecnicos: TStringList);
begin
  FTecnicos.Assign(ATecnicos);
end;

procedure TSmallStringGrid.SetHoras(AHoras: TStringList);
begin
  FHoras.Assign(AHoras);
end;

procedure TSmallStringGrid.SetSituacoes(ASituacoes: TStringList);
begin
  FSituacoes.Assign(ASituacoes);
end;

procedure TSmallStringGrid.SetSituacoesCor(ASituacoesCor: TStringList);
begin
  FSituacoesCor.Assign(ASituacoesCor);
end;


procedure TSmallStringGrid.AjustaHorario;
var
  i : Integer;
  HC: String;
begin
...
procedure TSmallStringGrid.CargaTecnicos;
begin
...
procedure TSmallStringGrid.SetCantoColor(Value: TColor);
begin
...
procedure TSmallStringGrid.SetHorariosColor(Value: TColor);
begin
...
procedure TSmallStringGrid.SetSelectColor(Value: TColor);
begin
...
procedure TSmallStringGrid.SetHorFonte(const Value: TFont);
begin
  FHorFonte.Assign(Value);
end;

procedure TSmallStringGrid.SetTitFonte(const Value: TFont);
begin
  FTitFonte.Assign(Value);
end;


procedure TSmallStringGrid.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  CR, AC, AR : Integer;
  C: TColor;
  HoraA,HoraI,HoraF: TTime;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if AllTrim(Ordens[CR].Hora_pro) <> '' then
        begin
...
procedure TSmallStringGrid.DrawCell(ACol, ARow: Integer; mRect: TRect;  AState: TGridDrawState);
var
  PC,CR: Integer;
  f,p: Double;
...
procedure TSmallStringGrid.SetTable(Value: TTable);
begin
   FTable := Value;
end;

function TSmallStringGrid.GetTable: TTable;
begin
  Result := FTable;
end;
...
procedure Register;
begin
  RegisterComponents('Small', [TSmallStringGrid]);
end;
