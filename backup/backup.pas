...
const
  ...
  cVersion = '3.00';
  cInsertDisk = 'Por favor, insira o disco %s e clique OK para continuar.';
  fSignature = 'EC2';
  fOldSignature = 'ECS';
  ...
  errCantreadFile = 'Não pode ler o arquivo.'+chr(10)+
                    'Pode estar sendo usado por outro usuário.';
  errCantwriteFile = 'Falha na substituição do arquivo';//Overwriting file failed';
  errCantreadArchive = 'Não pode ler o arquivo';//'Cannot read archive';
  errCantwriteArchive = 'Não pode gravar o arquivo';//'Cannot write to archive';
  errInvalidfiletype = 'Tipo de arquivo inválido';//'Invalid archive type';
  errCompression = 'Erro na compressão';//'Compression error';
  errCantCreateFileID = 'Não pode criar arquivo temporário para gravar IDs dos arquivos';//'Cannot create temporary file to save the file IDs';
...
//              if  (DiskFree(0)-10*1024) < size then break;
              ...
              if assigned(fOnProgress) then if (fSizeTotal div 100) > 0 then fOnProgress(self, CurrentFile, ((fProgressSize div 100)*100) div (fSizeTotal div 100), Continue);
        ...
        begin
            ...
            begin
              ...
              begin
                   ...
                   if Pos('__KYT',AnsiUpperCase(S)) = 0  then
                   begin
...
//            finally
//             SStr.free; //coloque para cima do finally
            end;
...
begin
{
     ...
     if assigned(fOnError) then fOnError(self, err, S) else MessageDlg(S, mtError, [mbOK], 0);
}
end;
...
//          else begin
          ...
          begin
            ...
            // Rogério 30/11/2004
            // Restaura o diretório de contatos
            if (Pos('\CONTATOS\',UpperCase(S))>0) and (not fRestoreFullPath) and (TargetPath <> '') then
               CurrentFile := TargetPath +'CONTATOS\'+ExtractFileName(S);
            ...
            begin
                 ...
                 begin
                    if MessageDlg('Não foi possível criar o diretório: '+chr(10)+chr(10)+
                                                  extractFileDir(CurrentFile)+chr(10)+chr(10)+
                                    'Deseja interromper ?'              ,
            ...
            try
              try
                // teste, se o arquivo existe tenta apagá-lo para dar erro se alguém estiver usando
                if FileExists(CurrentFile) then
                begin
                   if not DeleteFile(CurrentFile) then
                   begin
                      showmessage('Não foi possível restaurar o arquivo: '+chr(10)+chr(10)+
                                  CurrentFile+chr(10)+chr(10)+
                                  'Verifique se o mesmo está sendo usado por outro usuário ou sistema');
                      fLastErr := idCantReadFile;
                      Continue := false;
                   end;
                end;
              ...
              try             // coloquei para resolver o problema de Access violation...
                 FileSetDate(TStr.handle, FA);  //set original file date + time
                 TStr.free;
              except end;
            end
...
begin
  ...
  begin
    ...
    begin
       ...
       if assigned(fOnProgress) then fOnProgress(self, CurrentFile, ((fProgressSize div 100)*100) div (fSizeTotal div 100), Continue);
    end;
  end;
end;
