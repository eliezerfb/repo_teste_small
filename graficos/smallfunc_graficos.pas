unit smallfunc_graficos;

interface

uses
  SysUtils, DB, dialogs, windows, printers, xmldom,
  XMLIntf, MsXml, msxmldom, XMLDoc
  , IniFiles, DateUtils
  , StrUtils, Forms
  {$IFDEF VER150}// – Delphi 7
  , LbClass, BDE, DBTables, IBDatabase, IBQuery
   {$ELSE}
  , IBX.IBDataBase, IBX.IBQuery
  {$ENDIF}
  , Classes;

  const CHAVE_SMALL = 'FFEAA766654488992624076BDF9907FBBDEFF3CF616D352280FD6F0E13A59109D7761E3E0492EAB3DF38EB6D125451C36662933A3AC0D5AAC6AC4F926E89'+
...
//  function SMALL_IE(sNumero:String;UF:String):Boolean;
  ...
  {$ENDIF}
  function CriaTransaction(IBDATABASE: TIBDatabase; bAplicacao: Boolean = True): TIBTransaction;
  function CriaQuery(IBTRANSACTION: TIBTransaction): TIBQuery;
...
begin
  ...
  {$IFDEF VER150} // Compatibilizar com novas versão do Delphi Sandro Silva 2019-04-25
    //Compilando em Delphi 7.0'
    WinExec(PChar('TASKKILL /F /IM "'+sNomeExecutavel+'"'),SW_HIDE);
  {$ELSE}
    WinExec(PAnsiChar('TASKKILL /F /IM "'+sNomeExecutavel+'"'),SW_HIDE);
  {$ENDIF}
end;
...
begin
  ...
  for i := 1 to length(S) do begin
    ...
    while true do begin
      ...
      {$ELSE}
      if (S[i] = FormatSettings.DecimalSeparator) and (not SeenPoint) then begin
      {$ENDIF}      
        SeenPoint := true;
        break;
      end;
...
{    if not (S[i] in ['+', '-', '0'..'9', ' ', '_', ThousandSeparator]) then begin}
    ...
    {$ELSE}
    if not (S[i] in ['+', '-', '0'..'9', ' ', FormatSettings.ThousandSeparator]) then begin
    {$ENDIF}    
      Result := false;
      break;
    end;
...
begin
   ...
   begin
     ...
     {$ELSE}
     if dateToStr(pP1) = (StrPas(Feriados[I])+Copy(DateToStr(pP1),7,4)) then
     {$ENDIF}       
         Result:=False;
   end;
...
// Ver
  ...
  // Pelotas e região
  //
  // 2015-09-28 Foppa conversou com Ronei sobre o DDD do RS iniciar com 5, Pelotas é 53 if (Copy(sNumero,1,2) >= '43') then
...
begin
  //
{
  pP1 := StrTran(pP1,'&Aacute;','Á');
  pP1 := StrTran(pP1,'&aacute;','á');
  pP1 := StrTran(pP1,'&Acirc;','Â');
  pP1 := StrTran(pP1,'&acirc;','â');
  pP1 := StrTran(pP1,'&Agrave;','À');
  pP1 := StrTran(pP1,'&agrave;','à');
  pP1 := StrTran(pP1,'&Aring;','Å');
  pP1 := StrTran(pP1,'&aring;','å');
  pP1 := StrTran(pP1,'&Atilde;','Ã');
  pP1 := StrTran(pP1,'&atilde;','ã');
  pP1 := StrTran(pP1,'&Auml;','Ä');
  pP1 := StrTran(pP1,'&auml;','ä');
  pP1 := StrTran(pP1,'&AElig;','Æ');
  pP1 := StrTran(pP1,'&aelig;','æ');
  pP1 := StrTran(pP1,'&iuml;','ï');
  pP1 := StrTran(pP1,'&Eacute;','É');
  pP1 := StrTran(pP1,'&eacute;','é');
  pP1 := StrTran(pP1,'&Ecirc;','Ê');
  pP1 := StrTran(pP1,'&ecirc;','ê');
  pP1 := StrTran(pP1,'&Egrave;','È');
  pP1 := StrTran(pP1,'&egrave;','è');
  pP1 := StrTran(pP1,'&Euml;','Ë');
  pP1 := StrTran(pP1,'&euml;','ë');
  pP1 := StrTran(pP1,'&ETH;','Ð');
  pP1 := StrTran(pP1,'&eth;','ð');
  pP1 := StrTran(pP1,'&otilde;','õ');
  pP1 := StrTran(pP1,'&Iacute;','Í');
  pP1 := StrTran(pP1,'&iacute;','í');
  pP1 := StrTran(pP1,'&Icirc;','Î');
  pP1 := StrTran(pP1,'&icirc;','î');
  pP1 := StrTran(pP1,'&Igrave;','Ì');
  pP1 := StrTran(pP1,'&igrave;','ì');
  pP1 := StrTran(pP1,'&Iuml;','Ï');
  pP1 := StrTran(pP1,'&Ugrave;','Ù');
  pP1 := StrTran(pP1,'&ugrave;','ù');
  pP1 := StrTran(pP1,'&Oacute;','Ó');
  pP1 := StrTran(pP1,'&oacute;','ó');
  pP1 := StrTran(pP1,'&Ocirc;','Ô');
  pP1 := StrTran(pP1,'&ocirc;','ô');
  pP1 := StrTran(pP1,'&Ograve;','Ò');
  pP1 := StrTran(pP1,'&ograve;','ò');
  pP1 := StrTran(pP1,'&Oslash;','Ø');
  pP1 := StrTran(pP1,'&oslash;','ø');
  pP1 := StrTran(pP1,'&Otilde;','Õ');
  pP1 := StrTran(pP1,'&Yacute;','Ý');
  pP1 := StrTran(pP1,'&Ouml;','Ö');
  pP1 := StrTran(pP1,'&ouml;','ö');
  pP1 := StrTran(pP1,'&quot;','"');
  pP1 := StrTran(pP1,'&Uacute;','Ú');
  pP1 := StrTran(pP1,'&uacute;','ú');
  pP1 := StrTran(pP1,'&Ucirc;','Û');
  pP1 := StrTran(pP1,'&ucirc;','û');
  pP1 := StrTran(pP1,'&reg;','®');
  pP1 := StrTran(pP1,'&copy;','©');
  pP1 := StrTran(pP1,'&Uuml;','Ü');
  pP1 := StrTran(pP1,'&uuml;','ü');
  pP1 := StrTran(pP1,'&szlig;','ß');
  pP1 := StrTran(pP1,'&Ccedil;','Ç');
  pP1 := StrTran(pP1,'&ccedil;','ç');
  pP1 := StrTran(pP1,'&Ntilde;','Ñ');
  pP1 := StrTran(pP1,'&ntilde;','ñ');
  pP1 := StrTran(pP1,'&yacute;','ý');
//pP1 := StrTran(pP1,'&lt;','<');
//pP1 := StrTran(pP1,'&gt;','>');
...
begin
  ...
  try
    trAux := CriaTransaction(IBDATABASE);
    qyAux := CriaQuery(trAux);
    ...
    Blowfish.GenerateKey(CHAVE_SMALL); // Minha chave secreta
...
function CriaTransaction(IBDATABASE: TIBDatabase; bAplicacao: Boolean = True): TIBTransaction;
begin
  if bAplicacao = False then
    Result := TIBTransaction.Create(Application.MainForm)
  else
...
begin
  ...
  try
    // Sandro Silva 2019-11-20  Result := TIBQuery.Create(Application);
